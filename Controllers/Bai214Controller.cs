...
public class Bai214Controller : ControllerBase
{
    private readonly AdsMongoDbContext _context;

    public Bai214Controller(IConfiguration configuration, AdsMongoDbContext context)
    {
        _context = context;
    }
    ...
    public async Task<IActionResult> Create([FromBody] CalculationInputModel input)
    {
        ...
        double geometricMean = positiveNumbers.Any() ? Math.Pow(positiveNumbers.Aggregate(1.0, (prod, x) => prod * x), 1.0 / positiveNumbers.Count) : 0;
    ...
    public async Task<IActionResult> Update(string id, [FromBody] CalculationInputModel input)
    {
        ...
        existingResult.Answer = (int)geometricMean;
